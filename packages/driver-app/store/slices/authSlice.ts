...
export const loadStoredAuth = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const storedDriverStatus = await AsyncStorage.getItem('driver_status');
        ...
        if (testResponse.status === 401) {
          ...
          if (refreshData.success) {
            ...
            return {
              ...
              driverStatus: storedDriverStatus || 'offline'
            };
          } else {
            // Refresh token also expired
            await AsyncStorage.multiRemove(['auth_token', 'refresh_token', 'user_data', 'driver_status']);
  ...
  async ({ phone, password }: { phone: string; password: string }, { rejectWithValue }) => {
    try {
      ...
      if (data.success) {
        ...
        return { user, token, refreshToken, driverStatus: 'offline' };
      } else {
...
export const updateDriverStatus = createAsyncThunk(
  ...
  async ({ status, token }: { status: 'offline' | 'online' | 'busy'; token: string }, { rejectWithValue }) => {
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 30000);
      ...
      return rejectWithValue(error instanceof Error ? error.message : 'Status update failed');
    }
  }
);
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    setDriverStatus: (state, action: PayloadAction<'offline' | 'online' | 'busy'>) => {
    ...
    builder
      ...
      .addCase(loadStoredAuth.fulfilled, (state, action) => {
        ...
        if (action.payload) {
          ...
          state.driverStatus = action.payload.driverStatus as 'offline' | 'online' | 'busy';
        }
      })
...
export const { clearError, updateToken, updateUser, setDriverStatus } = authSlice.actions;
