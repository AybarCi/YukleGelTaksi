...
async function testDriverRegistration() {
  try {
    ...
    const response = await fetch(`${process.env.API_BASE_URL || 'http://localhost:3000'}/api/drivers/register`, {
      method: 'POST',
      headers: {
    ...
    if (error.code === 'ECONNREFUSED') {
      console.log('Make sure the backend server is running on', process.env.API_BASE_URL || 'http://localhost:3000');
    }
  }
}
