...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'cargo-type-photos');
    ...
    const fileName = `cargo-type-${timestamp}${fileExtension}`;
    ...
    // URL path'i oluştur
    const imageUrl = `/uploads/cargo-type-photos/${fileName}`;
    ...
    console.error('Cargo type image upload error:', error);
    return NextResponse.json(
